...
const Tecnologia = () => {
  ...
  useEffect(() => {
    const fetchProductos = async () => {
      try {
        ...
        const tecnologiaAPI = datos.filter(
          (producto) => producto.categoria === 'Tecnologia'
        );

        const tecnologiaCRUD = productosCRUD.filter(
          (producto) => producto.categoria === 'Tecnologia'
        );

        const idsCRUD = new Set(tecnologiaCRUD.map((p) => p.id));
        const apiSinDuplicar = tecnologiaAPI.filter(
          (p) => !idsCRUD.has(p.id)
        );

        setProductos([...tecnologiaCRUD, ...apiSinDuplicar]);
  ...
  return (
    <div style={{ padding: '20px' }}>
      <h2 style={{ color: '#FF6ECE', marginBottom: '20px' }}>Tecnología</h2>
